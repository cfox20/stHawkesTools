...
run_examples <- identical(Sys.getenv("NOT_CRAN"), "true") ||
...
```{r cran-note, echo = FALSE, eval = TRUE}
if (!run_examples) {
  knitr::asis_output(
    paste0(
      "**Note:** Code chunks in this vignette are not evaluated on CRAN to keep build times manageable. ",
      "Set `Sys.setenv(NOT_CRAN = \"true\")` before knitting to reproduce the results locally."
    )
  )
}
```
...
`stHawkesTools` is currently available from GitHub while the package is being
prepared for CRAN submission. Install the development version with `remotes` to
follow along with this vignette.
...
Once a release is published to CRAN the standard `install.packages()` workflow
